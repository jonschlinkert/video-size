...
function videoSizeSync(filepath) {
  ...
  const { error, status, stdout, stderr } = (0, import_child_process.spawnSync)("ffprobe", args);
  if (error) {
    throw error;
  }
  if (status !== 0) {
...
async function videoSize(filepath) {
  return new Promise((resolve, reject) => {
    const proc = (0, import_child_process.spawn)("ffprobe", [...defaultArgs, filepath]);
    let stdout = "";
    let stderr = "";
